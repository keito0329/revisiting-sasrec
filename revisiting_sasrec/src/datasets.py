...
class CausalLMPredictionDataset(LMDataset):
    ...
    def __init__(self, df, max_length=128, validation_mode=False,
                 shuffle_mode="original", shuffle_seed=None, padding_value=0,
                 user_col='user_id', item_col='item_id',
                 time_col='timestamp'):
        ...
        self.shuffle_mode = _normalize_shuffle_mode(shuffle_mode)
    ...
    def __getitem__(self, idx):
        ...
        if self.validation_mode:
            ...
            input_ids = _maybe_shuffle_sequence(
                input_ids,
...
class SuccessivePredictionDataset(LMDataset):
    ...
    def __init__(self, df, global_timepoint, max_length=128,
                 ...
                 shuffle_mode="original", shuffle_seed=None):
...
class MaskedLMPredictionDataset(LMDataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = _maybe_shuffle_masked_prefix(
            input_ids,
            mode=self.shuffle_mode,
            seed=self.shuffle_seed,
            idx=idx,
...
def _normalize_shuffle_mode(mode):
    if mode is None:
        return "original"
    mode = str(mode).lower()
    if mode in {"original", "none", "no"}:
        return "original"
    if mode in {"partial_shuffle_keep_last", "partial_keep_last", "partial_shuffle"}:
        return "partial_shuffle_keep_last"
    if mode in {"full_shuffle", "shuffle", "full"}:
        return "full_shuffle"
    raise ValueError(f"Unknown shuffle_mode: {mode}")


def _maybe_shuffle_sequence(values, mode, seed, idx, padding_value=0):
    if mode == "original":
        return values
    if len(values) <= 1:
        return values
    rng = np.random.default_rng(None if seed is None else int(seed) + int(idx))
    arr = np.array(values, dtype=np.int64, copy=True)
    positions = np.where(arr != padding_value)[0]
    if positions.size <= 1:
        return arr.tolist()
    if mode == "partial_shuffle_keep_last":
        shuffle_positions = positions[:-1]
    else:
        shuffle_positions = positions
    if shuffle_positions.size <= 1:
        return arr.tolist()
    shuffled_vals = arr[shuffle_positions].copy()
    rng.shuffle(shuffled_vals)
    arr[shuffle_positions] = shuffled_vals
    return arr.tolist()


def _maybe_shuffle_masked_prefix(values, mode, seed, idx):
    if mode == "original":
        return values
    if len(values) <= 2:
        return values
    prefix = _maybe_shuffle_sequence(
        values[:-1],
        mode=mode,
        seed=seed,
        idx=idx,
